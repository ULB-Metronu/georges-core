...
@pytest.mark.parametrize(
    "x, px, y, py, dpp, betax, alphax, betay, alphay, emitx, emity, dispx, dispy, dispxp, dispyp, dpprms",
    [
        (
            0.0 * _ureg.m,
            0.0,
...
def test_from_twiss(
    ...
    beam_distribution = Distribution.from_twiss_parameters(
        n=int(1e7),
        x=x,
        px=px,
        y=y,
    ...
    _np.testing.assert_allclose(
        ...
        _np.array(
            [
                emitx.m_as("m radians"),
                betax.m_as("m"),
                alphax,
                dispx.m_as("m"),
                dispxp,
                emity.m_as("m radians"),
                betay.m_as("m"),
                alphay,
                dispy.m_as("m"),
                dispyp,
...
@pytest.mark.parametrize(
    "x, px, y, py, dpp, xrms, pxrms, yrms, pyrms,dpprms",
    [
        (0.0 * _ureg.m, 0.0, 0.0 * _ureg.m, 0.0, 0.0, 0.1 * _ureg.cm, 0.0, 0.5 * _ureg.cm, 0.0, 0.0),
        (0.1 * _ureg.m, 0.0, 0.0 * _ureg.m, 0.0, 0.0, 0.0 * _ureg.m, 0.001, 0.003 * _ureg.m, 0.001, 0.001),
        (0 * _ureg.m, 5, 0.0 * _ureg.m, 0.0, 0.0, 0.6 * _ureg.cm, 0.1, 0.0 * _ureg.m, 0.0, 0.01),
    ],
)
def test_from_multigaussian_distribution(x, px, y, py, dpp, xrms, pxrms, yrms, pyrms, dpprms):
    beam_distribution = Distribution.from_5d_multigaussian_distribution(
        n=int(1e7),
        x=x,
        px=px,
        y=y,
...
@pytest.mark.parametrize(
    "x, px, y, py, dpp, s11, s12, s13, s14, s15, s22, s23, s24, s25, s33, s34, s35, s44, s45, s55",
    [
        (
            0.0 * _ureg.m,
            0.0,
...
@pytest.mark.parametrize(
    "x, px, y, py, dpp, matrix",
    [
        (0.0 * _ureg.m, 0.0, 0.0 * _ureg.m, 0.0, 0.0, _np.random.rand(5, 5)),
        (0.01 * _ureg.m, 0.0, 0.0 * _ureg.m, 0.0, 0.0, _np.random.rand(5, 5)),
    ],
)
