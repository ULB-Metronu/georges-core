...
from georges_core.kinematics import ekin_to_range, range_to_ekin
...
def test_single_proton():
    kin = compute_kinematics(1. * ureg.GeV, False, _Proton)
...
def test_single_electron():
    kin = compute_kinematics(1. * ureg.GeV, False, _Electron)
...
@pytest.mark.parametrize("quantity, kinetic", [(1200 * ureg.MeV, False),
                                               (100 * ureg.MeV, True),
                                               (100 * ureg.MeV, None),
                                               (5 * ureg.cm, True),
